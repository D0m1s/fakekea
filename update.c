...
void update() {
    ...
    sqlite3_stmt *stmt;
    sqlite3_prepare_v2(db, "select img, title, tag, description, price, tableCount, left1, right1, left2, right2, left3, right3, left4, right4, left5, right5, left6, right6, left7, right7, left8, right8, left9, right9, left10, right10 from product", -1, &stmt, NULL);

    sqlite3_stmt *stmt2;
    sqlite3_prepare_v2(db, "select count(*) from product", -1, &stmt2, NULL);
    sqlite3_step(stmt2);
    ...
    for (int i = 0; i < count; ++i) {
        ...
        addedSize = strlen(id);
        expand(&productHTML, strlen(productHTML) + 1, strlen(sqlite3_column_text(stmt3, 0)), offsets2[1] + addedSize, (char *)sqlite3_column_text(stmt3, 0));
        addedSize += strlen(sqlite3_column_text(stmt3, 0));
        expand(&productHTML, strlen(productHTML) + 1, strlen(sqlite3_column_text(stmt3, 1)), offsets2[2] + addedSize, (char *)sqlite3_column_text(stmt3, 1));
        addedSize += strlen(sqlite3_column_text(stmt3, 1));
        expand(&productHTML, strlen(productHTML) + 1, strlen(sqlite3_column_text(stmt3, 3)), offsets2[3] + addedSize, (char *)sqlite3_column_text(stmt3, 3));
        addedSize += strlen(sqlite3_column_text(stmt3, 3));
        if (sqlite3_column_int(stmt3, 5) != 0) {
            char tableBegin[8] = "<table>";
            expand(&productHTML, strlen(productHTML) + 1, 7, offsets2[3] + addedSize + 5, tableBegin);
            addedSize += 7;
            for (int j = 0; j < sqlite3_column_int(stmt3, 5); ++j) {
                char table[5] = "<tr>";
                char table1[5] = "<td>";
                char table2[6] = "</td>";
                char table3[6] = "</tr>";
                ...
                addedSize += 4;
                expand(&productHTML, strlen(productHTML) + 1, strlen(sqlite3_column_text(stmt3, 6 + j * 2)), offsets2[3] + addedSize + 5, (char *)sqlite3_column_text(stmt3, 6 + j * 2));
                addedSize += strlen(sqlite3_column_text(stmt3, 6 + j * 2));
                expand(&productHTML, strlen(productHTML) + 1, 5, offsets2[3] + addedSize + 5, table2);
                addedSize += 5;
                expand(&productHTML, strlen(productHTML) + 1, 4, offsets2[3] + addedSize + 5, table1);
                addedSize += 4;
                expand(&productHTML, strlen(productHTML) + 1, strlen(sqlite3_column_text(stmt3, 7 + j * 2)), offsets2[3] + addedSize + 5, (char *)sqlite3_column_text(stmt3, 7 + j * 2));
                addedSize += strlen(sqlite3_column_text(stmt3, 7 + j * 2));
                expand(&productHTML, strlen(productHTML) + 1, 5, offsets2[3] + addedSize + 5, table2);
                addedSize += 5;
        ...
        expand(&productHTML, strlen(productHTML) + 1, strlen(sqlite3_column_text(stmt3, 4)), offsets2[4] + addedSize, (char *)sqlite3_column_text(stmt3, 4));
        expand(&templateHTML, strlen(templateHTML) + 1, strlen(productHTML), offset, productHTML);
        offset += strlen(productHTML);
        free(productHTML);
    }
